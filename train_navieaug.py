...
import torchvision
from networks.data_loader import Beta_BSDS, Beta_BSDSnaive
...
# this file is used for analysing model scaling
# use dataload Beta_BSDSnaive for naive aug data
# data in/out
out_dir = 'beta_PiDiNet_naivedata'
dataset = '../BetaBSDS'
model_name = 'PiDiNet' # RCF HED PiDiNet
...
def main():
    ...
    # dataset
    train_dataset = Beta_BSDSnaive(root=dataset, model = model_name, split="train")
    test_dataset = Beta_BSDSnaive(root=dataset, split="test")
...
def train(train_loader, model, optimizer, epoch, save_dir):
    ...
    for i, (image, label, total, dists) in enumerate(train_loader):
        ...
        for idxo, o in enumerate(outputs):
            ...
            loss_know = loss_know + beta_knowledge(o, dists[:, idxo:idxo + 1]) * W_know / itersize
        counter += 1
